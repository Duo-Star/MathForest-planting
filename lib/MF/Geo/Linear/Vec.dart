...
class Vec {
  ...
  num operator ^(num index) => math.pow(x,index) + math.pow(y,index) + math.pow(z,index);
  ...
  num ang(Vec other) => math.acos(cosAngle(other).clamp(-1.0, 1.0));// 计算与另一个向量的夹角（弧度，范围 0 到 π）

  Vec tp(Vec p, Vec u, Vec v, Vec n) => p + u * x + v * y + n * z;
  Vec tp2d(Vec p,Vec u,Vec v) => p + u * x + v * y;

  Vec roll(Vec n , num w)=> scale(math.cos(w)) + (n.cross(this)).scale(math.sin(w)) + n.scale( (n.dot(this)) * (1-math.cos(w)) );
  ...
  @override
  bool operator ==(Object other) => other is Vec && x == other.x && y == other.y && z == other.z;
